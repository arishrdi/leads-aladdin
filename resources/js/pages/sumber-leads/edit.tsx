...
import { ArrowLeft, Save, Database, Info, Activity } from 'lucide-react';
...
interface PageProps {
    sumberLead: SumberLeads;
}

export default function EditSumberLeads() {
    const { sumberLead } = usePage<PageProps>().props;
    
    const { data, setData, patch, processing, errors } = useForm({
        nama: sumberLead.nama,
        deskripsi: sumberLead.deskripsi || '',
        is_active: sumberLead.is_active,
    });

    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        patch(`/sumber-leads/${sumberLead.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Sumber Leads: ${sumberLead.nama} - Leads Aladdin`} />
            ...
            <div className="max-w-3xl mx-auto space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-brand-primary">Edit Sumber Leads: {sumberLead.nama}</h1>
                        <p className="text-gray-600">
                            Perbarui informasi sumber leads
                        </p>
                    </div>
                </div>
                ...
                {/* Warning for active sources with leads */}
                {sumberLead.leads_count && sumberLead.leads_count > 0 && (
                    <Card>
                        <CardHeader>
                        ...
                        <CardContent>
                            <div className="p-4 bg-amber-50 rounded-lg">
                                <p className="text-amber-800 text-sm">
                                    <strong>Sumber leads ini memiliki {sumberLead.leads_count} leads yang terkait.</strong>
                                </p>
                                <p className="text-amber-700 text-sm mt-1">
                                    Menonaktifkan sumber leads tidak akan menghapus leads yang sudah ada, 
                                    tetapi sumber ini tidak akan tersedia untuk leads baru.
                                </p>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <ul className="space-y-1 text-gray-600">
                                    ...
                                    <li>• Pastikan nama baru jelas dan konsisten</li>
                                </ul>
                            </div>
                            <div>
                                ...
                                <ul className="space-y-1 text-gray-600">
                                    ...
                                    <li>• Leads lama tetap mempertahankan sumber</li>
