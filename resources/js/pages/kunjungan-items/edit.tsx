...
interface KunjunganItem {
    ...
    urutan: number;
    is_active: boolean;
    kunjungan_item_category_id: number;
    kunjungan_item_category?: KunjunganItemCategory;
}
...
interface PageProps {
    category: KunjunganItemCategory & {
        kunjungan_items: KunjunganItem[];
    };
}

export default function EditKunjunganItem() {
    ...
    const { data, setData, patch, processing, errors } = useForm({
        ...
        input_type: category.input_type || 'checkbox',
        urutan: category.urutan || 0,
        is_active: category.is_active,
        items: [] as ItemData[],
    });

    // Initialize items from category data
    useEffect(() => {
        if (category.kunjungan_items) {
            const formattedItems: ItemData[] = category.kunjungan_items.map(item => ({
                id: item.id,
                nama: item.nama || '',
    ...
    const updateItem = (itemId: string | number, field: keyof ItemData, value: any) => {
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        patch(`/kunjungan-items/category-${category.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit Kategori - Leads Aladdin" />
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Perbarui kategori "{category.nama}" dan kelola item-itemnya
                        </p>
                    </div>
                </div>
                    ...
                    <Card className="border-border/50 shadow-soft">
                        <CardContent className="pt-6">
                            <div className="flex flex-col sm:flex-row gap-4">
                                ...
                                >
                                    {processing ? 'Menyimpan...' : 'Perbarui Kategori & Item'}
                                </Button>
                            </div>
                        </CardContent>
                    </Card>
